...
const API_URL = "https://onlinewriting.onrender.com/api/open/registration-fee";
      ...
      try {
        ...
        if (response.ok && Array.isArray(data)) {
          const filtered = data.filter(
            (item) => item.role !== "string" && item.amount !== "string"
          );
          setLimits(filtered);
        } else {
          throw new Error("Failed to fetch registration limits.");
        }
      } catch (error) {
        toast({
          title: "Fetch Error",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok && data.id) {
        ...
        setLimits((prev) =>
          editingId
            ? prev.map((item) => (item.id === editingId ? newLimit : item))
            : [...prev, newLimit]
        );
